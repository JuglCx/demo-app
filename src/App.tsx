...
import { loadConfigs } from "./lib/loadConfig";
...
function SafeImage({
  ...
  const fallbackSiteName = siteName || "Website";
  const fallbackText = label || `${fallbackSiteName} Preview`;
...
export default function App() {
  ...
  useEffect(() => {
    const brand = window.location.pathname.split("/")[1] || "jugl";
    ...
    async function init() {
      try {
        const { apiConfig, chatbotConfig, siteConfig } =
          await loadConfigs(brand);

        setConfigs({ apiConfig, chatbotConfig, siteConfig });
  ...
  const { siteConfig } = configs as { siteConfig: SiteConfig };
